...
def encrypt_data(final_package):
    ...
    try:
        # JSON সাজানোর জন্য indent ব্যবহার করা হয়েছে
...
def push_to_github():
    print("[-] GitHub এ শুধুমাত্র fawna.json আপডেট পাঠানো হচ্ছে...")
    try:
        os.system('git config --global user.email "api00007@gmail.com"')
        os.system('git config --global user.name "iVann-flux"')
        
        # গিটহাব থেকে পুল করে সিঙ্ক করা
        os.system("git pull origin main --rebase")
        
        # শুধুমাত্র fawna.json ফাইলটা অ্যাড করা
        os.system(f"git add {OUTPUT_FILE}")
        
        # কমিট এবং পুশ
        os.system(f'git commit -m "Auto Update: {get_ist_time()}" || echo "No change"')
        os.system("git push origin main")
        print("[SUCCESS] GitHub এ শুধুমাত্র ডাটা ফাইল আপডেট হয়েছে।")
    except Exception as e:
...
def run_automation():
                            ...
                            try:
                                ...
                                for idx, m3u8 in enumerate(m3u8_links, 1):
                                    match_list.append({
                                        ...
                                        "Rivels": rivals,
                                        "Title": f"{title} (Stream {idx})" if len(m3u8_links) > 1 else title,
    ...
    if found_any:
        # --- আপনার ট্রেডমার্ক সাজানো হচ্ছে ---
        # OrderedDict ব্যবহার করা হয়েছে যাতে সিরিয়াল ঠিক থাকে
